-- ============================================================================
-- TeamMap — Complete fix: defines helper functions + creates member_sheets
-- Run this standalone (does not depend on migration.sql having run).
-- Safe to re-run.
-- ============================================================================

-- ── 1. Define helper functions (from schema.sql) ────────────────────────────
-- These must exist before any RLS policy references them.
...
-- ── 2. Ensure profiles has needed columns ───────────────────────────────────
alter table profiles add column if not exists email text;
alter table profiles add column if not exists invited_at timestamptz;

-- ── 3. Create member_sheets table (if not already created) ──────────────────
...
-- ── 4. Drop any stale policies (table now exists, so IF EXISTS works) ───────
...
-- ── 5. Recreate member_sheets RLS policies ──────────────────────────────────
...
-- ── 6. Recreate tasks INSERT policy for members ────────────────────────────
...
-- ── 7. Drop old helper functions before recreating ──────────────────────────
...
-- ── 8. Recreate admin helper functions ──────────────────────────────────────
